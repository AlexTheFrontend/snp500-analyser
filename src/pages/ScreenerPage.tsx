...
const SECTORS = ['All', 'Financials', 'Healthcare', 'Consumer Staples', 'Consumer Discretionary', 'Energy', 'Industrials', 'Technology', 'Materials', 'Utilities', 'Communications']
...
export default function ScreenerPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return result
  }, [stocks, sector, rating, minScore, sortKey, sortDir])
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="bg-bg-card border border-bg-border rounded-xl p-4 mb-4 flex flex-wrap items-end gap-4">
        ...
        <div className="ml-auto flex items-center gap-3">
          <span className="text-xs text-text-secondary font-mono">{filtered.length} stocks</span>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-3 text-text-secondary text-sm">
          ...
          <span>Fetching data for {stocks.length > 0 ? stocks.length : '44'} stocks from Yahoo Finance...</span>
          <span className="text-xs text-text-tertiary">This may take 10-20 seconds on first load</span>
        </div>
      ) : error ? (
